...
节点名称修改和删除只作用于本地筛选结果，不会回写原订阅；启用 Gist 时会在本地保存成功后自动同步 Gist。GUI 输出文件名决定远程配置身份：不同文件名分别创建独立 Gist 并获得不同 Gist ID 和稳定 raw 链接，同名文件再次同步时才覆盖原内容且保持链接不变。服务器、端口或凭据不开放编辑，因为连接参数变化后旧测速结果会立即失效。旧版设置中的 `NodeNotes` 会在启动时自动永久清除。
